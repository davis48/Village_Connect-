...
app.get('/api/admin/stats', authMiddleware(['admin']), async (req, res) => {
  try {
    const [[bornesStats]] = await pool.query(`
      SELECT
        COUNT(*) AS total,
        SUM(statut = 'actif') AS actives,
        SUM(statut = 'panne') AS pannes,
    ...
    const [[usersStats]] = await pool.query(`
      SELECT
        ...
        SUM(statut_connexion = 'connecte') AS connectes
      FROM utilisateurs
    `);
    ...
    const [[revenusStats]] = await pool.query(`
      SELECT
        ...
        COALESCE(SUM(IF(created_at >= DATE_FORMAT(NOW(), '%Y-%m-01'), montant, 0)), 0) AS mois,
        COALESCE(SUM(IF(created_at >= DATE_SUB(NOW(), INTERVAL 3 MONTH), montant, 0)), 0) AS trimestre
      FROM ventes
    `);

    const [[vouchersStats]] = await pool.query(`
      SELECT
        COUNT(*) AS total,
        SUM(utilise = 1) AS utilises,
        SUM(utilise = 0 AND expiration > NOW()) AS disponibles,
        SUM(utilise = 0 AND expiration <= NOW()) AS expires,
        SUM(MONTH(created_at) = MONTH(NOW()) AND YEAR(created_at) = YEAR(NOW())) AS mois
      FROM codes_acces
    `);

    const [formulesRows] = await pool.query(`
      SELECT formule, COUNT(*) AS total
      FROM ventes
      GROUP BY formule
    `);

    const [[alertesStats]] = await pool.query(`
      SELECT SUM(lu = 0) AS non_lues
      FROM alertes
    `);

    const objectifMensuel = Number(process.env.REVENU_OBJECTIF_MENSUEL || 500000);
    const objectifPct = objectifMensuel > 0
      ? Math.min(100, Math.round((Number(revenusStats.mois || 0) / objectifMensuel) * 100))
      : 0;

    const formules = { journalier: 0, hebdomadaire: 0, mensuel: 0 };
    formulesRows.forEach((r) => {
      formules[r.formule] = Number(r.total || 0);
    });

    res.json({
      bornes: {
        total: Number(bornesStats.total || 0),
        actives: Number(bornesStats.actives || 0),
        pannes: Number(bornesStats.pannes || 0),
        maintenance: Number(bornesStats.maintenance || 0),
      },
      utilisateurs: {
        total: Number(usersStats.total_utilisateurs || 0),
        connectes: Number(usersStats.connectes || 0),
      },
      revenus: {
        mois: Number(revenusStats.mois || 0),
        trimestre: Number(revenusStats.trimestre || 0),
        total: Number(revenusStats.total || 0),
        objectif_mensuel: objectifMensuel,
        objectif_pct: objectifPct,
      },
      vouchers: {
        total: Number(vouchersStats.total || 0),
        utilises: Number(vouchersStats.utilises || 0),
        disponibles: Number(vouchersStats.disponibles || 0),
        expires: Number(vouchersStats.expires || 0),
        mois: Number(vouchersStats.mois || 0),
      },
      formules,
      alertes: {
        non_lues: Number(alertesStats.non_lues || 0),
      },
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
